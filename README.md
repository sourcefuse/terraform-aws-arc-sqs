...
| <a name="input_dlq_config"></a> [dlq\_config](#input\_dlq\_config) | Dead Letter Queue configuration. Set enabled=true to create DLQ | <pre>object({<br/>    enabled                     = optional(bool, false)<br/>    name                        = optional(string, null)<br/>    max_receive_count           = optional(number, 3)<br/>    message_retention_seconds   = optional(number, 1209600)<br/>    delay_seconds               = optional(number, 0)<br/>    max_message_size            = optional(number, 262144)<br/>    receive_wait_time_seconds   = optional(number, 0)<br/>    visibility_timeout          = optional(number, 30)<br/>    content_based_deduplication = optional(bool, false)<br/>    deduplication_scope         = optional(string, null)<br/>    throughput_limit            = optional(string, null)<br/>    sqs_managed_sse_enabled     = optional(bool, true)<br/>    redrive_allow_policy        = optional(string, null)<br/>  })</pre> | <pre>{<br/>  "enabled": false<br/>}</pre> | no |
| <a name="input_dlq_tags"></a> [dlq\_tags](#input\_dlq\_tags) | Additional tags specific to the dead letter queue | `map(string)` | `{}` | no |
| <a name="input_encryption_config"></a> [encryption\_config](#input\_encryption\_config) | Encryption configuration for the queue | <pre>object({<br/>    kms_encryption_enabled       = optional(bool, false)<br/>    existing_kms_key_id          = optional(string, null)<br/>    kms_data_key_reuse_period    = optional(number, 300)<br/>    sqs_managed_sse_enabled      = optional(bool, true)<br/>    create_kms_key               = optional(bool, false)<br/>    kms_key_deletion_window_days = optional(number, 7)<br/>    kms_key_rotation_enabled     = optional(bool, true)<br/>    kms_key_alias                = optional(string, null)<br/>    kms_key_policy               = optional(string, null)<br/>    kms_key_tags                 = optional(map(string), {})<br/>  })</pre> | `{}` | no |
...
| <a name="input_queue_tags"></a> [queue\_tags](#input\_queue\_tags) | Additional tags specific to the main queue | `map(string)` | `{}` | no |
...
| <a name="input_use_name_prefix"></a> [use\_name\_prefix](#input\_use\_name\_prefix) | Determines whether to use `name_prefix` or `name` for the queue name | `bool` | `false` | no |
