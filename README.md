...
|------|-------------|------|---------|:--------:|
| <a name="input_create_key"></a> [create\_key](#input\_create\_key) | Whether to create a new KMS key for queue encryption. Ignored if kms\_key is provided | `bool` | `false` | no |
...
| <a name="input_kms_config"></a> [kms\_config](#input\_kms\_config) | Configuration for creating a new KMS key. Only used if create\_key=true and kms\_key=null | <pre>object({<br/>    data_key_reuse_period = optional(number, 300)<br/>    deletion_window_days  = optional(number, 7)<br/>    rotation_enabled      = optional(bool, true)<br/>    alias                 = optional(string, null)<br/>    policy                = optional(string, null)<br/>  })</pre> | `null` | no |
| <a name="input_kms_key"></a> [kms\_key](#input\_kms\_key) | ARN of existing KMS key for queue encryption. If null and create\_key=false, uses AWS managed SSE-SQS encryption | `string` | `null` | no |
